import SwiftUI

struct Palette {
    static let background = Color(hex: "FFFFFF")
    static let backgroundDark = Color(hex: "0D0D0D")
    
    static let text = Color(hex: "111111")
    static let textDark = Color(hex: "EAEAEA")
    
    static let textSecondary = Color(hex: "666666")
    static let textSecondaryDark = Color(hex: "999999")
    
    static let success = Color(hex: "34C759")
    static let error = Color(hex: "FF3B30")
    static let warning = Color(hex: "FF9500")
    static let highlight = Color(hex: "FFD447")
    
    static let border = Color(hex: "CCCCCC")
    static let borderDark = Color(hex: "333333")
    
    static let cardBackground = Color(hex: "F8F8F8")
    static let cardBackgroundDark = Color(hex: "1C1C1C")
    
    static let accent = Color(hex: "007AFF")
}

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3:
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6:
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8:
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (255, 0, 0, 0)
        }
        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue: Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}
